...
export class GuideJobrequestsComponent {
  dataSource: string[] = [];
  displayedColumns: string[] = ['date', 'customerId', 'placeCount', 'action'];
  ...
  getJobs(id: any) {
    this.guideService.getJobs(id).subscribe((response) => {
  ...
  showPlaces(id: string) {
    ...
    this.placeService
      ...
      .subscribe((response) => {
        console.log('dialog data - ',response.placeList);
